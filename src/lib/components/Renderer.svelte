<script lang='ts'>
import { GUI_HISTORY, GUI_LOAD, GUI_SAVE, GUI_SETTINGS, lovebars, LOVE_BAR_BG, LOVE_BAR_BROKEN_HEART, LOVE_BAR_EMPTY, nameboxes } from "$lib/assets";
  ...
  async function render() {
    ...
    if (scene.background_visible) {
      ...
      ctx.translate(scene.background_offset_x * (-xMoveMax) - xMoveMax, scene.background_offset_y * (-yMoveMax) - yMoveMax)
    ...
    for(const char of scene.characters) {
      ...
      ctx.drawImage(img, 1920 / 2 + char.image_offset_x * (1920/2) - (img.width * char.image_offset_scale) / 2, char.image_offset_y * (1920/2) + img.width - img.width * char.image_offset_scale, img.width * char.image_offset_scale, img.height * char.image_offset_scale);
    }
    ...
    if (scene.lovebar_visible) {
      ...
      ctx.drawImage(loveBarFull, 0, 0, loveBarFull.width * scene.lovebar_amount, loveBarFull.height, 777, 10, 315 * scene.lovebar_amount, 38);
