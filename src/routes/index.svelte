...
<main>
  <div class="editor">
    ...
    <p>
      This is a mock screenshot / meme generator for the hit video game<br> <a href="https://store.steampowered.com/app/1703610/I_Just_Want_to_be_Single/">I Just Want To Be Single</a>. Assest ripped from game <a href="/proof.png">with permission.</a>
    </p>
      ...
      {#if chosen_character === 'Custom'}
        ...
        <select bind:value={$scene.namebox_color} disabled={!$scene.textbox_visible}>
          {#each Object.keys(nameboxes) as color}
            <option value={color}>{color.slice(0, 1).toUpperCase()}{color.slice(1).toLowerCase()}</option>
          {/each}
        </select>
    ...
    <div class="hsplit">
      <div class="row">
        ...
        <input class='small-input' type="number" bind:value={$scene.background_offset_x} max="1" min="-1">
      </div>
      <div class="row">
        <p>Y: </p>
        <input type="range" bind:value={$scene.background_offset_y} max="1" min="-1" step="0.01">
        <input class='small-input' type="number" bind:value={$scene.background_offset_y} max="1" min="-1">
      </div>
    </div>

    <p>Note: 0, 0 is center. 1 or -1 makes image touch edge, based on scale. This makes it so the image is always covering the bg.</p>
    ...
    {#each $scene.characters as character}
      <div class="character-editor">
        <div class="hsplit">
          <div class="row">
            <select bind:value={character.character} on:change={(ev) => {
              character.image = characters.find(x => x.name === ev.currentTarget.value).sprites[0].url;
            }}>
              {#each characters as char}
                <option value={char.name}>{char.name}</option>
              {/each}
        ...
        <div class="hsplit">
          <div class='row' style='flex:3'>
            <p>X:</p>
            <input type="range" bind:value={character.image_offset_x} max="0.9" min="-0.9" step="0.01">
            <input class='small-input' type="number" bind:value={character.image_offset_x} max="10" min="-10">
          </div>
        </div>
        <div class="hsplit">
          <div class="row">
            <p>Y:</p>
            <input type="range" bind:value={character.image_offset_y} max="0.5" min="-0.5" step="0.01">
            <input class='small-input' type="number" bind:value={character.image_offset_y} max="10" min="-10">
          </div>
          <div class="row">
            <p>Scale:</p>
            <input type="range" bind:value={character.image_offset_scale} max="1.5" min="0.5" step="0.01">
            <input class='small-input' type="number" bind:value={character.image_offset_scale} max="4" min="0">
          </div>
        </div>
      </div>
    {/each}
